...
default_setting = {'verbose': False, 'use_stereo': True, 'use_symbol': True, 'max_unmap': 5, 'retro': True,
...
def reassign_atom_mapping(transform, atom_dict):
    ...
    for all_labels in p_labels:
        ...
        if RETRO:
            transform_newmaps = transform_newmaps.split('>>')[0] + '>>' + '.'.join(
                [inv_temp(smarts) for smarts in transform_newmaps.split('>>')[1].split('.')])
        else:
...
def split_reagents(reaction):
    ...
    least_atom_n = min(
        [max([Chem.MolFromSmiles(smiles).GetNumAtoms() for smiles in ps if smiles not in rs]), LEAST_ATOM_NUM])
...
def extract_from_reaction(reaction, setting=default_setting):
    ...
    product_maps = [atom.GetAtomMapNum() for products in products_list for atom in
                    Chem.MolFromSmiles(products).GetAtoms()]
    ...
    atom_dict = {str(atom.GetAtomMapNum()): {'charge': atom.GetFormalCharge(), 'Hs': atom.GetNumExplicitHs()} for atom
                 in changed_atoms}
    ...
    edits, H_change, Charge_change, Chiral_change = match_label(reactants_smiles, products_smiles, replacement_dict,
                                                                changed_atom_tags, retro=RETRO, remote=REMOTE,
                                                                use_stereo=USE_STEREOCHEMISTRY)
